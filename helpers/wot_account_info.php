...
if (isset($_POST["search"])) {
    ...
    $account_list_response = json_decode(grab("$url_wot_account_list?application_id=$application_id&search=$nickname&limit=1"), true);
    $account_id = $account_list_response["data"][0]["account_id"];
    
    if (isset($account_id)) {
        $account_info_response = json_decode(grab("$url_wot_account_info?application_id=$application_id&account_id=$account_id"), true);
        $tanks_stat_response = json_decode(grab("$url_wot_tanks_stat?application_id=$application_id&account_id=$account_id"), true);
        $account_achievements_response = json_decode(grab("$url_wot_account_achievements?application_id=$application_id&account_id=$account_id"), true);
        ...
        foreach ($sliced_tanks_array as $tanks_array) {
            ...
            $tanks_encyclopedia_response = json_decode(grab("$url_wot_encyclopedia?application_id=$application_id&tank_id=$tanks_ids&fields=tier,name,tank_id"), true);
            $tanks_encyclopedia += $tanks_encyclopedia_response["data"];
        }

        $clan_info = [];
        $clans_accountinfo_response = json_decode(grab("$url_wot_clans_accountinfo?application_id=$application_id&account_id=$account_id"), true);
        ...
        if (isset($clan_id)) {
            ...
            $clans_info_response = json_decode(grab("$url_wot_clans_info?application_id=$application_id&clan_id=$clan_id"), true);
            ...
            $clan_description = $clans_info_response["data"][$clan_id]["description_html"];
            $clan_emblems = $clans_info_response["data"][$clan_id]["emblems"];
            ...
            $clan_info += array(
                ...
                "description" => $clan_description, "emblems" => $clan_emblems
            );
        }

        $glossary_response = json_decode(grab("$url_wot_glossary?application_id=$application_id"), true);
