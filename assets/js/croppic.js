			...
			that.form.find('input[type="file"]').change(function(){
				...
				if(that.options.processInline) {			
				    ...
				    if (typeof FileReader == "undefined"){
						...
						var input_file = that.form.find('input[type="file"]').val();
						var extension = input_file.split('.').pop().toLowerCase();
						var isValidFormat = fileTypes.indexOf(extension) > -1;
						...
						if (isValidFormat === true) {
							...
							that.reset();
							$(".croppic-next").hide();
							$(".croppic-error").html("Invalid image format");
						}						
					}
				} else {		
